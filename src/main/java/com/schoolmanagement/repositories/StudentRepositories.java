...
public interface StudentRepositories extends PagingAndSortingRepository<User, Integer> {
  ...
  @Query(value = "SELECT u " + "FROM User u " + "INNER JOIN UserRole AS ur ON u.id = ur.pk.userId "
      ...
      + "WHERE ur.pk.roleId = 4 AND u.aClass.id = :id")
  ...
  @Query(value = "SELECT u " + "FROM User u " + "INNER JOIN UserRole ur ON ur.pk.userId = u.id "
      + "INNER JOIN Role r ON r.id = ur.pk.roleId "
      + "INNER JOIN UserInfo ui ON ui.id = u.userInfo.id "
  ...
  @Query(value = "SELECT u " + "FROM User u " + "INNER JOIN UserRole ur ON ur.pk.userId = u.id "
      ...
      + "INNER JOIN UserInfo ui ON ui.id = u.userInfo.id "
      + "WHERE ur.pk.roleId = 4 AND u.aClass IS NULL "
      + "AND ui.admissionYear = :admissionYear - :grade + 10")
  ...
  @Query(value =
      "SELECT new com.schoolmanagement.model.request.MarkRequest(u.id , u.fullName , u.dob , u.address , m.coefficient , m.id) "
          + "FROM User u " + "JOIN Mark m ON u.id = m.studentId " + "WHERE m.type = :type "
          + "AND m.semester = :semester " + "AND m.subjectId = :subjectId "
          + "AND u.aClass.id = :classId")
  ...
  @Query(value = "SELECT COUNT(u.id) " + "FROM user AS u "
      ...
      + "WHERE ur.role_id = 4 "
      + "AND ui.admission_year = YEAR(DATE_SUB(CURDATE(), INTERVAL 1 YEAR))"
      + "", nativeQuery = true)
  ...
  @Query(value = "SELECT COUNT(u.id) " + "FROM user AS u "
      ...
      + "inner join role as r on r.id = ur.role_id  "
      + "INNER JOIN user_info AS ui ON u.user_info_id = ui.id "
      + "INNER JOIN class AS c ON c.id = u.class_id "
      + "WHERE ur.role_id = 4 "
//			+ "AND c.grade = 12 "
      + "AND ui.status = 3 "
  ...
  @Query(value = "SELECT COUNT(id) "
      + "FROM mark  "
      + "WHERE `type` = 6 AND coefficient < 5 "
//			+ "AND YEAR(created_date) = YEAR(DATE_SUB(CURDATE(), INTERVAL 1 YEAR)) "
      + "", nativeQuery = true)
  ...
  @Query(value = "SELECT COUNT(u.id) "
      ...
      + "INNER JOIN user_info AS ui ON u.user_info_id = ui.id "
      + "WHERE ur.role_id = 4 "
      + "AND ui.admission_year = :year "
      + "", nativeQuery = true)
  int countAllNewStudentByYear(@Param("year") int year);
  ...
  @Query(value = "SELECT COUNT(u.id) " + "FROM user AS u "
      ...
      + "AND c.grade = 12 "
      + "AND ui.status = 3 "
      + "AND ui.graduate_year = :year ", nativeQuery = true)
  int countAllStudentGraduateByYear(@Param("year") int year);
  ...
  @Query(value = "SELECT COUNT(u.id) "
      ...
      + "AND ui.status = 2 "
      + "AND ui.graduate_year = :year ", nativeQuery = true)
  int countAllStudentAbsentByYear(@Param("year") int year);

  @Query(value = "SELECT COUNT(id) "
      + "FROM mark  "
      + "WHERE `type` = 6 AND coefficient < 5 "
      + "AND YEAR(created_date) = :year ", nativeQuery = true)
  int countAllStudentFailByYear(@Param("year") int year);
