...
public interface BlogRepositories extends PagingAndSortingRepository<Blog , Integer> {
	...
	@Query(value = "SELECT b FROM Blog b WHERE b.header LIKE %:header% AND b.createdDate >= :createdDate")
	 Page<Blog> findAllByHeaderAndFromDate(@Param("header") String header, @Param("createdDate")
			LocalDate fromDate, Pageable pageable);

	@Query(value = "SELECT b FROM Blog b WHERE b.header LIKE %:header% AND b.createdDate <= :createdDate")
	Page<Blog> findAllByHeaderAndToDate(@Param("header") String header, @Param("createdDate")
